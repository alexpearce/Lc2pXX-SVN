...
from lc2pxx import config, ntuples, fitting
...
def efficiencies(mode, polarity, year):
    ...
    return {
        ...
        "Tracking": tracking.efficiency(mode, polarity, year),
        "Stripping": stripping.efficiency(mode, polarity, year),
        # "Trigger": trigger.efficiency_tistos(mode, polarity, year),
        # "Trigger": trigger.efficiency(mode, polarity, year),
        "Trigger": trigger.efficiency_post_stripping(mode, polarity, year),
        "Offline": offline.efficiency(mode, polarity, year)
    }


# TODO verbose flag? prints the individual efficiencies
def branching_fractions(polarity, year):
...
if __name__ == "__main__":
    # Redefine this to exclude the pKS mode, as we're not interested in
    # it here
